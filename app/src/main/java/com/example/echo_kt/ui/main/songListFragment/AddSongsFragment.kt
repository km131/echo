        ...
        binding.apply {
            setAddSongs {
                lifecycleScope.launch(Dispatchers.IO) {
                    val isCheckedList =
                        (rvSongs.adapter as MultipleChoiceListAdapter).mSelectedPositions
                    ...
                    AppDataBase.getInstance().customSongListDao()
                        .insertPlaylistsWithSongs(playlistWithSongs)
                    AppDataBase.getInstance().customSongListDao()
                        .updateSongList(playList.apply { number += playlistWithSongs.size })
                    withContext(Dispatchers.Main) {
                        Toast.makeText(BaseApplication.getContext(), "添加成功", Toast.LENGTH_SHORT)
                            .show()
